...
namespace EffSln.EtradeSdkTest
{
    public abstract class MenuConsoleApp : IHostedService
    {
        ...
        public async Task HandleMenu(Dictionary<int, (string Description, Func<Task> Action)> options, string menuTitle)
        {
            bool running = true;
            ...
            while (running)
            {
                ...
                {
                    await options[choice].Action.Invoke(); // Invoke the corresponding action
                    if (choice == 0)
                    {
        ...
        public static Dictionary<int, (string Description, Func<Task> Action)> BuildMenuOptions<T>(
            IEnumerable<T> items,
            Func<T, string> displaySelector,
            Func<T, Func<Task>> actionSelector)
        {
            // Map each item to a key-value pair for the menu dictionary
            return items
                .Select((item, index) => new KeyValuePair<int, (string, Func<Task>)>(
                    index + 1, // Menu options are 1-based
                    (displaySelector(item) ?? $"Account {index + 1}", actionSelector(item))
                ))
